...
if role == "Sender":
    ...
    if st.button("Send Message"):
        try:
            ...
            with st.sidebar:
                add_text1 = st.markdown("# Key Generation")
                add_text2=st.markdown("## Sender key:")
                add_text2_info=st.info(f"{sender_key}")
                add_text3=st.markdown("## ECC Public key:")
                add_text3_info=st.info(f"{ecc_public_key}")
                add_text4=st.markdown("# Encryption:")
                add_text5=st.markdown("## AES Ciphertext")
                add_text5_info=st.info(f"{ciphertext}")
                add_text6=st.markdown("## AES tag:")
                add_text6_info=st.info(f"{tag}")
